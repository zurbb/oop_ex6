...
public class GlobalVariablesValidator {
    ...
    private static void verifyTypeMatchValue(String type, String value) throws VerifierExceptions.ValueNotMatchToType{
        ...
        // double
        else if(type.equals(Types.DOUBLE) && RegexUtils.DOUBLE_VALUE.matcher(value).find()){
            return;
        }
        // boolean
        else if(type.equals(Types.BOOLEAN) && RegexUtils.BOOLEAN_VALUE.matcher(value).find()){
            return;
        }
        //char
        else if(type.equals(Types.CHAR) && RegexUtils.CHAR_VALUE.matcher(value).find()){
            return;
        }
        //string
        else if(type.equals(Types.STRING) && RegexUtils.STRING_VALUE.matcher(value).find()) {
            return;
        }
